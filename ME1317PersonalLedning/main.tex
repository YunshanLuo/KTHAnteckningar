...
\newpage
\section{Sammanfattning av artiklarna inför KS-1}
\subsection{Artikel 1: Barry, M., T. Dundon, A. Wilkinson (2018)}
\subsubsection{Employee voice}
\begin{itemize}
    \item Analyserar medarbetaröst genom flera discipliner:
    \begin{itemize}
        \item Organizational Behavior (OB)
        \item Human Resource Management (HRM)
        \item Employee Relations (ER)
        \item Labour Process Theory (LPT)
    \end{itemize}
\end{itemize}

\subsection{Organisational Behavior (OB)}
\begin{itemize}
    \item Antar en \textit{unitarisk syn}, arbetarna och ledningen har samma intressen
    \item Rösten är individuellt, frivilligt och "prosocialt" rösten ses som ett verktyg som gynnar organisationen 
    \item Fokuserar på mikronivå (individens perspektiv) och \textit{antecedenter}.
    \item Antecedenter är 
\end{itemize}
