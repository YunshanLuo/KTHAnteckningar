...
\begin{itemize}
    ...
    \item \textbf{Lemmatisering} := Göra ord till grundform i.e dess lemma \textit{eg. springer $\rightarrow$ springa}
    \item \textbf{Lemma} := Grundformen av ett ord
    \item \textbf{Substantiv} := Ord på saker och ting \textit{eg. En \textbf{basketboll}}
    \item \textbf{Verb} := Saker som man gör \textit{eg. \textbf{springa}}
    \item \textbf{Adjektiv} := Beskriver saker och ting \textit{eg. \textbf{snabb} som en bil}
...
\begin{itemize}
    ...
    \item NP := Nominalfras (Noun Phrase) eg. \textit{apa, en apa, apan på basketplanen}
...
\subsection*{Binär logistisk regreesion}
\begin{itemize}
    \item Feature vektorn: $\overline{x} = (x_1, x_2, \dots, x_n)$
    \item Modellen har vikiter: $\overline{\theta} = (\theta_0, \theta_1, \dots, \theta_n)$
    \item $h(\overline{x}) = \sigma(\overline{\theta}^T \overline{x})$, Sannolikehten att x tillhör den positiva klassen
    \item $\sigma(z) = \frac{e^z}{1+e^z}$, Sigmoid funktionen
\end{itemize}

Visualisering av logistisk regression i neuralt nätverk:
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{nn.png}
\end{figure}

\subsection*{Linjär separerbarhet}
AND är linjärt separerbar medan XOR är inte linjärt separerbar
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{linear.png}
\end{figure}
Med flera artificiala neuroner kan man lösa icke linjärt separerbara problem såsom XOR

\newpage
\subsection*{XOR i neuralt nätverk}
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{xor.png}
\end{figure}
XOR truth table:
\begin{center}
\begin{tabular}{cc|c}
...
\end{tabular}
\end{center}
Beräkning av $\theta$ och $\lambda$ för XOR:
$$\lambda_1 \sigma (\theta_{11} x_1 + \theta_{12} x_2 + \theta_{10}) + \lambda_2 \sigma(\theta_{21} x_1 + \theta_{22} x_2 + \theta_{20}) + \lambda_0$$

\[
\begin{cases}
    \lambda_1 \sigma (\theta_{11} + \theta_{12} + \theta_{10}) + \lambda_2 \sigma(\theta_{21} + \theta_{22} + \theta_{20}) + \lambda_0 \approx 0 & \text{, om } x_1 = 1 \text{ och } x_2 = 1 \\
    \lambda_1 \sigma (\theta_{11} + \theta_{10}) + \lambda_2 \sigma(\theta_{21} + \theta_{20}) + \lambda_0 \approx 1 & \text{, om } x_1 = 1 \text{ och } x_2 = 0 \\
    \lambda_1 \sigma (\theta_{12} + \theta_{10}) + \lambda_2 \sigma(\theta_{22} + \theta_{20}) + \lambda_0 \approx 1 & \text{, om } x_1 = 0 \text{ och } x_2 = 1 \\
    \lambda_1 \sigma (\theta_{10}) + \lambda_2 \sigma(\theta_{20}) + \lambda_0 \approx 0 & \text{, om } x_1 = 0 \text{ och } x_2 = 0 \\
\end{cases}
\]

För enkelhetenskull kan vi gruppera operationer till neuroner så att det ser ut såhär:
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{xor2.png}
\end{figure}
Den första lagret kallas för hidden layer och den andra lagret kallas för output layer.

\subsection*{Beräkning av XOR nätverket med matris multiplikation}
Hidden layer beräknas:
$$a = \sigma(\theta x)$$
Output layer beräknas:
$$\tilde{y} = \sigma(\lambda a)$$

Med siffror kan det se ut såhär:
$$\theta = \begin{bmatrix}
    -3 & -5 & \phantom{-}5 \\
    -5 & \phantom{-}5 & -5
\end{bmatrix}$$
...
Sedan beräknas $a$ och $\tilde{y}$:
$$a = \sigma(\theta x + b_{\theta})$$
$$\tilde{y} = \sigma(\lambda a + b_{\lambda})$$

\subsection*{Feed forward networks}
För många gömda lager kan det beräknas på följande sätt:
$$z_1 = \theta_1 x$$
$$a_1 = g(z_1)$$
...
$g$ är någon icke linjär funktion såsom sigmoid och softmax
\newline
\vspace{10pt}
Sammanfattning av feed forward nätverk:
\begin{itemize}
    \item Nätverket beräknar någon icke funktion av inputten
    \item Varje lager beräknar en linjär och icke linjär transformation
    \item Nätverket kan sammanfattningsvis beskrivas som $f = f_1 \circ f_2 \circ \dots \circ f_n$
    \item Varje $f_i$ är en linjär och icke linjär transformation och $f_i$ är viktad med $\theta_i$ som tränas
\end{itemize}

\subsection*{Aktiverings funktioner}
Det finns alternativ till sigmoid och softmax såsom:
\begin{itemize}
    \item Rectified Linear Unit (ReLU): $ReLU(z) = \max(0, z)$
    \item Hyperbolic tangent (tanh): $tanh(z) = \frac{e^z - e^{-z}}{e^z + e^{-z}}$
\end{itemize}

\subsection*{Inlärning}
I logistisk regression används gradient descent med två steg:
\begin{itemize}
    \item Ett framåt steg för att beräkna predicerade värdet
    \item Ett bakåt steg för att beräkna förlusten 
\end{itemize}

Anta att vi har en loss funktion $L(\theta)$ då är gradienten: 
$$\nabla_{\theta} L = \begin{bmatrix}
    \frac{\partial L}{\partial \theta_0} \\[0.5em]
    \frac{\partial L}{\partial \theta_1} \\[0.5em]
    \vdots \\[0.5em]
    \frac{\partial L}{\partial \theta_n}
\end{bmatrix}$$
\newpage
\subsection*{Backpropagation}
Exempel: Givet att loss är $L$
$$L(x,y,z) = (x+y)z$$
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{backprop.png}
\end{figure}

\begin{itemize}
    \item Vi får via ett framåt pass att $L(-2,5,-4) = -12$
\end{itemize}

För att göra en backpropagation måste vi beräkna derivatorna:
$$q(x,y) = x + y, \hspace{10pt} \frac{\partial q}{\partial x} = 1, \hspace{10pt} \frac{\partial q}{\partial y} = 1$$
$$L(q,z) = qz, \hspace{10pt} \frac{\partial L}{\partial q} = z, \hspace{10pt} \frac{\partial L}{\partial z} = q$$
Vi vill hitta $\frac{\partial L}{\partial x}, \hspace{5pt} \frac{\partial L}{\partial y}, \hspace{5pt} \frac{\partial L}{\partial z}$
$$\frac{\partial L}{\partial q} = z = -4$$
$$\frac{\partial L}{\partial z} = q = 3$$
$$\frac{\partial L}{\partial y} = \frac{\partial L}{\partial q} \cdot \frac{\partial q}{\partial y} = z \cdot 1 = -4 \cdot 1 = x+y = -4$$
$$\frac{\partial L}{\partial x} = \frac{\partial L}{\partial q} \cdot \frac{\partial q}{\partial x} = z \cdot 1 = -4 \cdot 1 = x+y = -4$$

\newpage
\subsection*{Korsentropi loss function}
Antag att vi har $\theta = [-0.1; 0.5; -0.3]$ och $x = [1; 1]$ gör ett framåt pass här:
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{cross.png}
\end{figure}
\begin{itemize}
    \item Framåt pass ger oss $\approx 0.744$
    \item Bakåt pass ger oss $\approx 0.525$ för alla
\end{itemize}:

\subsection*{Neurala språkmodeller}
$$\text{Word embeddnings} \rightarrow \text{Hidden layer} \rightarrow \text{Output layer}$$
\begin{itemize}
    \item Hidden layer: $z = \sigma(W_1 x)$
    \item Output layer: $ \text{logit} \hspace{5pt} \tilde{y} = \sigma(W_2 z), \hspace{10pt} softmax(\tilde{y}) = P(w_{n+1} | w_1,..,w_n)$
\end{itemize}

Genom att träna neurala nätverket och minimera loss via forward och backword propagation kan vi kan vi få fram sannolikehterna för nästa ord

\subsection*{Sampling tekniker}
Tekniker att välja nästa ord:
\begin{itemize}
    \item Greedy decoding: Alltid välja det mest sannolika outputten
    \item Random sampling: Välja nästa ord baserat på sannolikheten
    \item Top-k sampling: Slumpar $k$ vanligaste orden och väljer sedan baserat på sannolikheten
    \item Top-p (nucleus sampling) sampling: Väljer de minsta mängden ord vars sannolikhet är större än $p$ och väljer sedan baserat på sannolikheten
\end{itemize}

\subsection*{Temperatur sampling}
Temperatur är $T$:
\begin{itemize}
    \item Genom att räkna ut $softamx(\frac{\tilde{y}}{T})$
    \item Då jämnas sannolikeheten ut och det blir mer sannolikt att välja mindre sannolika ord 
    \item Det vill säga att variansen på variansen minskar när $T$ ökar
\end{itemize}
