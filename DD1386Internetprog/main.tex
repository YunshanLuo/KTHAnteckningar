...
% Justera radavstånd till något läsbart
\renewcommand{\baselinestretch}{1.1}

% ... dina marginalinställningar ...
...
% Samlad konfiguration för minted
\setminted{
    linenos,
    breaklines,
    frame=lines,
    fontsize=\normalsize,
    bgcolor=bg,
    style=default % Monokai är snyggt, men kräver mörk bakgrund för att synas bra
}

\usemintedstyle{friendly} % 'friendly' eller 'colorful' passar bättre på ljus bakgrund än monokai
\begin{document}
...
3 delar i labb 2
\begin{enumerate}
    \item Textbaserat med nodejs (ingen HTTP-server)
    \item Grafisk html bara i webbläsaren (ingen HTTP-server)
    \item Liten webbserver i nodejs server (med HTTP-server)
\end{enumerate}

Spelflöde och dokumentation:
\begin{enumerate}
    \item Skapa spelplan -- $createGameplan(row,col)$
    \item Skapa spelare -- $createPlayer(gameplan)$
    \item Välj ruta -- $selectCell(plyaer, turn, gameplan)$
    \item Uppdatera spelplan -- $Update(gameplan, sign, row, col)$
    \item Kontrollera vinnare -- $checkWinner(checkWinner, row, col, minInLine2Win)$
    \item Kontrollera om spelplanen är full -- $noEmptyCells(gameplan)$
    \item Byt tur till nästa spelare -- Löser automatiskt
    \item Upprepa spelflödet -- Löser automatiskt
\end{enumerate}

Tester för spelet:
\begin{itemize}
    \item $createGameplanTest()$
    \item $updateTest()$
    \item $noEmptyCellsTest()$
    \item $checkWinnerTest()$
\end{itemize}

\section{Föreläsning 6}
\subsection{Nodejs}
\begin{itemize}
    \item Platform för att skapa serverapplikationer i JavaScript
    \item Innehåller allt man behöver för att skapa en webbserver
\end{itemize}
...
\begin{itemize}
    ...
    \item Hanterar tredjepartsbibliotek och verktyg
\end{itemize}

Exempel på enkel Nodejs server:
\begin{minted}{javascript}
    //file: serverhttp.js
...
Förklaring av koden:
\begin{itemize}
    \item $require$ - Importerar moduler i detta fall http
    \item const $createServer$ - Gör basically följande i ett steg
    \begin{enumerate}
        \item let http\_modulen = require("http");
        \item let createServer = http-\_modulen.createServer;
        \item (request, response) => {...} - Callback-funktion som körs varje gång en request kommer in
        \item server.listen(8000) - Startar servern och lyssnar på port 8000
        \item console.log("Listening! (port 8000)") - Skriver ut i konsolen att servern är igång
    \end{enumerate}
\end{itemize}

Exempel med modulen URL:
\begin{minted}{javascript}
    import http from "http";
    const server = http.createServer((req, res) => {
        res.writeHead(200, {"Content-Type": "text/plain"})
        res.write("resurs" + req.url);
        res.end();
    });
    server.listen(3000, () => {
        console.log("Servern är igång på port http://localhost:3000");
    });
\end{minted}

Exempel med url parameter name:
\begin{minted}{javascript}
    import http from "http";
    import {URL} from "url";
    const server = http.createServer((req, res) => {
        ...
        res.writeHead(200, {"Content-Type": "text/plain"});
        res.write("pathname: " + myURL.pathname + "\n");
        res.write("name: " + myURL.pathname + "\n");
        res.write("name: " + myURL.searchParams.get("name") + "\n");
        res.end();
    })
    server.listen(3000, () => {
        console.log("Servern är igång på port http://localhost:3000");
        console.log("prova med http://127.0.0.1:3000/index.html?name=KTH")
    });
\end{minted}

\section{Föreläsning 7}
%Fortsätt på föreläsning 7 imorgon
